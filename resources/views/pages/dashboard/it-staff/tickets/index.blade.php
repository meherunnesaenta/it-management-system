{{-- resources/views/it-staff/tickets/index.blade.php --}}
...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-white font-space">Assigned Tickets</h1>
            <p class="text-white/40 text-sm mt-1">Tickets assigned to you for resolution</p>
        </div>
        <div class="flex items-center gap-3">
            <span class="inline-flex items-center px-3 py-1.5 bg-white/5 border border-white/10 rounded-lg text-sm text-white/60">
                <span class="w-2 h-2 rounded-full bg-yellow-400 mr-2"></span>
                {{ $tickets->where('status', '!=', 'resolved')->where('status', '!=', 'closed')->count() }} Active
            </span>
        </div>
    </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div class="bg-white/5 backdrop-blur-sm border border-white/10 rounded-xl p-4">
            <p class="text-white/40 text-sm">Total Assigned</p>
            ...
            <table class="w-full">
                <thead class="bg-white/5">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left text-xs font-medium text-white/40 uppercase tracking-wider">Created By</th>
                        ...
                        <tr class="hover:bg-white/5 transition-colors duration-200" 
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('it-staff.tickets.show', $ticket) }}" class="p-2 text-white/30 hover:text-white hover:bg-white/5 rounded-lg transition-all duration-200" title="View">
                                    ...
                                    @if($ticket->status != 'resolved' && $ticket->status != 'closed')
                                        <form method="POST" action="{{ route('it-staff.tickets.update-status', $ticket) }}" class="inline">
                                            @csrf
                                            @method('PUT')
                                            <select name="status" onchange="this.form.submit()" class="px-2 py-1 bg-white/5 border border-white/10 rounded-lg text-xs text-white/60 hover:text-white focus:border-[#FF6B35] transition-all duration-200 appearance-none">
                                                <option value="open" class="bg-[#1A1A2E]" {{ $ticket->status == 'open' ? 'selected' : '' }}>Open</option>
                                                <option value="in-progress" class="bg-[#1A1A2E]" {{ $ticket->status == 'in-progress' ? 'selected' : '' }}>In Progress</option>
                                                <option value="resolved" class="bg-[#1A1A2E]" {{ $ticket->status == 'resolved' ? 'selected' : '' }}>Resolved</option>
                                            </select>
                                        </form>
                                    @endif
                                </div>
                        ...
                        <tr>
                            <td colspan="7" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-white/40 text-lg">No tickets assigned to you</p>
                                    <p class="text-white/20 text-sm mt-1">When tickets are assigned, they'll appear here</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
