{{-- resources/views/it-staff/tickets/show.blade.php --}}
...
<div class="max-w-5xl mx-auto space-y-6">
    {{-- ব্যাক বাটন --}}
    <a href="{{ route('it-staff.tickets.index') }}" class="inline-flex items-center gap-2 text-white/40 hover:text-white transition-colors duration-200">
            ...
            <div class="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                <div class="flex items-start gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-white/40 text-sm mt-1">
                            Created {{ $ticket->created_at->diffForHumans() }} by 
                            <span class="text-white/60">{{ $ticket->user?->name ?? 'Unknown' }}</span>
                        </p>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-2 flex-wrap">
                    @if($ticket->status != 'resolved' && $ticket->status != 'closed')
                        <form method="POST" action="{{ route('it-staff.tickets.resolve', $ticket) }}" class="inline">
                            @csrf
                            @method('PUT')
                            <button type="submit" class="px-4 py-2 bg-green-500/20 hover:bg-green-500/30 text-green-400 border border-green-500/30 rounded-xl text-sm font-medium transition-all duration-200">
                    ...
                    @if($ticket->status == 'open')
                        <form method="POST" action="{{ route('it-staff.tickets.start', $ticket) }}" class="inline">
                            @csrf
                            @method('PUT')
                            <button type="submit" class="px-4 py-2 bg-blue-500/20 hover:bg-blue-500/30 text-blue-400 border border-blue-500/30 rounded-xl text-sm font-medium transition-all duration-200">
                                <span class="flex items-center gap-2">
                                    <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="bg-white/5 backdrop-blur-sm border border-white/10 rounded-xl p-4">
            <p class="text-white/40 text-xs uppercase tracking-wider">Created By</p>
            <p class="text-white font-medium mt-1">{{ $ticket->user?->name ?? 'Unknown' }}</p>
        </div>
    </div>
        ...
        <div class="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl p-8">
            ...
            <div class="space-y-4 max-h-96 overflow-y-auto pr-2 custom-scrollbar">
                ...
                @empty
                    <div class="text-center py-8">
                        ...
                        <p class="text-white/20 text-sm">Be the first to respond</p>
                    </div>
                @endforelse
            </div>
            ...
            @if($ticket->status != 'closed')
                <div class="mt-6 pt-6 border-t border-white/5">
                    <form method="POST" action="{{ route('it-staff.tickets.respond', $ticket) }}" class="space-y-4">
                        @csrf
                        <div>
                            <label for="response" class="block text-sm font-medium text-white/70 mb-1.5">
                                Add Response
                            </label>
                            <textarea 
                                id="response" 
                        ...
                        <div class="flex justify-end">
                            <button type="submit" class="px-6 py-2.5 bg-gradient-to-r from-[#FF6B35] to-[#FF8F65] text-white font-semibold rounded-xl shadow-lg shadow-[#FF6B35]/25 hover:shadow-[#FF6B35]/40 hover:-translate-y-0.5 active:scale-[0.98] transition-all duration-300">
                                <span class="flex items-center gap-2">
                                    <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
            ...
            @else
                <div class="mt-6 pt-6 border-t border-white/5">
                    <p class="text-white/30 text-sm text-center">This ticket is closed. No further responses can be added.</p>
                </div>
            @endif
        </div>
    </div>
