...
<div class="max-w-5xl mx-auto space-y-6">
    {{-- ব্যাক বাটন --}}
    <a href="{{ route('admin.tickets.index') }}" class="inline-flex items-center gap-2 text-white/40 hover:text-white transition-colors duration-200">
            ...
            <div class="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                ...
                <div class="flex items-center gap-2 flex-wrap">
                    @if($ticket->status != 'resolved' && $ticket->status != 'closed')
                        <form method="POST" action="{{ route('admin.tickets.resolve', $ticket) }}" class="inline">
                            @csrf
                            @method('PUT')
                    ...
                    @endif
                    <a href="{{ route('admin.tickets.edit', $ticket) }}" class="px-4 py-2 bg-white/5 hover:bg-white/10 text-white/60 hover:text-white border border-white/10 rounded-xl text-sm font-medium transition-all duration-200">
                        <span class="flex items-center gap-2">
                            <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="bg-white/5 backdrop-blur-sm border border-white/10 rounded-xl p-4">
            ...
            <p class="text-white font-medium mt-1">{{ $ticket->assignedTo?->name ?? 'Unassigned' }}</p>
        </div>
    </div>
        ...
        <div class="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl p-8">
            ...
            @if($ticket->status != 'closed')
                <div class="mt-6 pt-6 border-t border-white/5">
                    <form method="POST" action="{{ route('admin.tickets.respond', $ticket) }}" class="space-y-4">
                        @csrf
                        <div>
