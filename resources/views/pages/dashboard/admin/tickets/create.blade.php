    ...
    <div class="relative">
        ...
        <div class="relative bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl p-8">
            ...
            <form method="POST" action="{{ route('admin.tickets.store') }}" class="space-y-6">
                ...
                {{-- অ্যাসাইন টু (ঐচ্ছিক) --}}
                <div>
                    <label for="assigned_to" class="block text-sm font-medium text-white/70 mb-1.5">
                        Assign To (Optional)
                    </label>
                    <select 
                        id="assigned_to" 
                        ...
                        class="w-full px-4 py-2.5 bg-white/5 border border-white/10 rounded-xl text-white placeholder-white/30 focus:border-[#FF6B35] focus:ring-2 focus:ring-[#FF6B35]/50 focus:ring-offset-2 focus:ring-offset-[#0A0A0F] transition-all duration-200 appearance-none"
                    >
                        <option value="" class="bg-[#1A1A2E]">Unassigned</option>
                        @foreach($staffUsers ?? [] as $staff)
                            <option value="{{ $staff->id }}" class="bg-[#1A1A2E]" {{ old('assigned_to') == $staff->id ? 'selected' : '' }}>
                ...
                <div class="flex flex-col sm:flex-row items-center justify-end gap-4 pt-6 border-t border-white/5">
                    <a href="{{ route('admin.tickets.index') }}" class="w-full sm:w-auto px-6 py-2.5 text-sm font-medium text-white/60 hover:text-white bg-white/5 hover:bg-white/10 rounded-xl transition-all duration-200 text-center">
                        Cancel
                    </a>
                    <button 
                        type="submit" 
